...
static int dashcdg_rx_apply_audio_frame_locked(
...

    {
        ...
        if (host_ch == 2U && ses_sr != 0U && out_sr != 0U) {
            size_t out_fc =
                    ((size_t) decoded_frames * (size_t) out_sr + (size_t) ses_sr - 1U) / (size_t) ses_sr;
    ...
    while (combined_steps < DASHCDG_MAX_DRAIN_STEPS_PER_CALL * 2U) {
        ...
        if (state->audio_jitter.initialized) {
            ...
            if (step == DASHCDG_AUDIO_DRAIN_SKIP) {
                ...
                if (apply_rc == 0) {
                    /* Device buffer full: keep the jitter slot and do not advance CDG this pass. */
...
#pragma GCC diagnostic ignored "-Wformat"
    ...
    fprintf(
            ...
            " grp=%u/%u parity=%u/%u hot=%u/%u | audio buffered=%ums decode_fail=%" DASHCDG_RX_PRIu64
...
static void dashcdg_rx_configure_audio_locked(
    ...
    fprintf(
            stdout,
            "[rx] audio: output ring (session_sr=%u pa_open_request_hz=%u wire_ch=%u host_ch=%u frame_ms=%u preroll=%u prof=%u codec=%u)\n",
            (unsigned int) sample_rate,
...
#pragma GCC diagnostic ignored "-Wformat"
    ...
    if (g_receiver.announced_transport_version == DASHCDG_PROTOCOL_VERSION_V4) {
        ...
        snprintf(
                hud_line_b,
                hud_line_b_size,
                "pre:%u/%u pend:%u/%u hot:%u/%u mute:%s | %.20s | %.20s"
...
static void display(void) {
    ...
    if (g_receiver.last_datagram_local_ms == 0U) {
        show_connecting = 1;
    } else if (dashcdg_rx_elapsed_ms_safe(local_now_ms, g_receiver.last_datagram_local_ms) >=
            DASHCDG_STREAM_LOSS_CONNECTING_MS) {
        show_connecting = 1;
        reconnecting = 1;
    }
    pthread_mutex_unlock(&g_receiver.mutex);
