...
**`dashcdg_badge` (current):** `esp_lcd` ST7789 + `atanisoft/esp_lcd_touch_xpt2046` + `esp_lvgl_port` / LVGL 9; touch UI for Wi-Fi scan/SSID/password (NVS), auto-reconnect from saved credentials. GPIO map and touch flags (**`CYD_TP_MIRROR_X`**, **`CYD_TP_TOUCH_INVERT_LVGL_Y`**, four-corner NVS cal): **`main/board_cyd_freenove_32.h`**. **Karaoke** screen: v4 UDP multicast CDG proof (shared **`proto/`** + **`core/`** via `components/dashcdg_*`). CI builds this app when **`platform/espidf/`** changes (see root **`.gitlab-ci.yml`**).
