/*
 * Audio lab (Applications): Mary demo + status strip. Speaker/touch NVS lives in audio_settings_ui.
 */
#include <stdbool.h>
#include <stdio.h>
#include <string.h>
...
/** Match `karaoke_ui` header-to-body gap (no CDG slot; keeps title/play under the status bar). */
#define AUDIO_LAB_HEADER_TO_BODY_GAP_PX 10
...
static void lab_status_line_set(void)
{
    ...
    if (s_ym_playing) {
        lv_label_set_text(s_bar_line, LV_SYMBOL_AUDIO "  Mary demo");
...
static void on_info_btn(lv_event_t *e)
{
    ...
    lv_label_set_text(body,
                      "Change speaker level and touch chirps under Setup, tile Audio and touch beeps. "
                      "Touch UI tones stay muted on this screen.");
...
static void on_play_pause(lv_event_t *e)
{
    ...
    if (s_play_lbl && lv_obj_is_valid(s_play_lbl)) {
        lv_label_set_text(s_play_lbl,
                          s_ym_playing ? LV_SYMBOL_PAUSE "  Pause demo" : LV_SYMBOL_PLAY "  Play demo");
    }
    lab_status_line_set();
}
...
static void on_back(lv_event_t *e)
{
    ...
    /* Before rebuilding Apps, drop AUDIO_LAB gate in platform_hw (touch triads check s_screen). */
...
esp_err_t dashcdg_audio_lab_ui_present(lv_disp_t *disp)
{
    ...
    lv_obj_set_style_pad_top(scroll, AUDIO_LAB_HEADER_TO_BODY_GAP_PX, 0);
    lv_obj_set_style_border_width(scroll, 0, 0);
    lv_obj_set_style_bg_opa(scroll, LV_OPA_TRANSP, 0);
    lv_obj_set_scroll_dir(scroll, LV_DIR_VER);
    ...
    lv_obj_t *hdr = lv_label_create(scroll);
    lv_label_set_text(hdr, LV_SYMBOL_AUDIO "  Audio lab");
    lv_obj_set_style_text_color(hdr, lv_color_hex(0x88ffcc), 0);
    ...
    lv_obj_set_style_border_color(card, lv_color_hex(0x1a4a3a), 0);
    lv_obj_set_style_radius(card, 8, 0);
    lv_obj_set_style_bg_color(card, lv_color_hex(0x040807), 0);
    ...
    lv_obj_t *setup_hint = lv_label_create(card);
    lv_label_set_long_mode(setup_hint, LV_LABEL_LONG_MODE_WRAP);
    lv_obj_set_width(setup_hint, lv_pct(100));
    lv_obj_set_style_text_color(setup_hint, lv_color_hex(0x8899aa), 0);
    lv_label_set_text(setup_hint,
                        "Speaker + touch chirps: main -> Setup gear -> Audio and touch beeps. "
                        "Demo level follows saved speaker level.");

    lv_obj_t *demo_sec = lv_label_create(card);
    lv_label_set_text(demo_sec, "DEMO TUNE");
    lv_obj_set_style_text_color(demo_sec, lv_color_hex(0x7a9f8e), 0);
    ...
    lv_obj_set_height(play_btn, 48);
    lv_obj_set_style_bg_color(play_btn, lv_color_hex(0x0c2218), 0);
    lv_obj_set_style_bg_grad_color(play_btn, lv_color_hex(0x143528), 0);
    lv_obj_set_style_bg_grad_dir(play_btn, LV_GRAD_DIR_VER, 0);
    lv_obj_set_style_border_color(play_btn, lv_color_hex(0x44bb88), 0);
    ...
    lv_label_set_text(s_play_lbl, LV_SYMBOL_PLAY "  Play demo");
    lv_obj_set_style_text_color(s_play_lbl, lv_color_hex(0xeefffa), 0);
