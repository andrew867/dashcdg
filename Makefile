...
# Optional ncurses panel for desktop-tx (runtime: DASHCDG_TX_UI=ncurses). MSYS2: mingw-w64-*-ncurses / libncursesw.
DASHCDG_TX_NCURSES ?= 0
TX_UI_NCURSES_OBJ := $(OBJ_DIR)/tx_ui_ncurses.o
ifeq ($(DASHCDG_TX_NCURSES),1)
CFLAGS += -DDASHCDG_TX_UI_NCURSES=1
TX_NCURSES_LDLIBS ?= -lncursesw
else
TX_NCURSES_LDLIBS :=
endif
...
TEST_NB_CODEC_ADAPTERS_BIN := $(BIN_DIR)/test-nb-codec-adapters
...
$(OBJ_DIR)/tx_ui_ncurses.o: platform/desktop/src/tx_ui_ncurses.c
	$(CC) $(CFLAGS) -c -o $@ $<
...
$(OBJ_DIR)/test_nb_codec_adapters.o: tests/test_nb_codec_adapters.c
	$(CC) $(CFLAGS) -c -o $@ $<
...
$(PLAYER_BIN): $(OBJ_DIR)/app_desktop_player.o $(DESKTOP_RX_GL_OBJECT) $(DESKTOP_TX_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ)
	$(CC) $(CFLAGS) $(EXTRA_LDFLAGS) -o $@ $(OBJ_DIR)/app_desktop_player.o $(DESKTOP_RX_GL_OBJECT) $(DESKTOP_TX_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ) $(LDLIBS_DESKTOP) $(NET_LIBS) $(WINDOWS_GDI_LIBS) $(TX_NCURSES_LDLIBS)
ifneq ($(WINDOWS_RUNTIME_DLLS),)
	cp -f $(WINDOWS_RUNTIME_DLLS) $(BIN_DIR)/
endif
	@if [ "$(MINGW_ARCH)" = "mingw32" ] && [ -f "$(BIN_DIR)/libopus.dll" ] && [ ! -f "$(BIN_DIR)/libopus-0.dll" ]; then cp -f "$(BIN_DIR)/libopus.dll" "$(BIN_DIR)/libopus-0.dll"; fi

$(TX_BIN): $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_LINK_OBJ) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ)
ifeq (,$(filter Windows_NT MINGW64_NT% MINGW32_NT% MSYS_NT%,$(OS) $(UNAME_S)))
	$(CC) $(CFLAGS) $(EXTRA_LDFLAGS) -o $@ $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_LINK_OBJ) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ) $(LDLIBS_DESKTOP) $(NET_LIBS) $(TX_NCURSES_LDLIBS)
else
	$(CC) $(CFLAGS) $(EXTRA_LDFLAGS) -o $@ $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_LINK_OBJ) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ) $(LDLIBS_DESKTOP_RX_GDI) $(TX_NCURSES_LDLIBS)
endif
ifneq ($(WINDOWS_RUNTIME_DLLS),)
	cp -f $(WINDOWS_RUNTIME_DLLS) $(BIN_DIR)/
endif
...
ifneq ($(TX_GDI_BIN),)
$(TX_GDI_BIN): $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_GDI_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ)
	$(CC) $(CFLAGS) $(EXTRA_LDFLAGS) -o $@ $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_GDI_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ) $(LDLIBS_DESKTOP_TX_GDI) $(TX_NCURSES_LDLIBS)
ifneq ($(WINDOWS_RUNTIME_DLLS),)
	cp -f $(WINDOWS_RUNTIME_DLLS) $(BIN_DIR)/
endif
	@if [ "$(MINGW_ARCH)" = "mingw32" ] && [ -f "$(BIN_DIR)/libopus.dll" ] && [ ! -f "$(BIN_DIR)/libopus-0.dll" ]; then cp -f "$(BIN_DIR)/libopus.dll" "$(BIN_DIR)/libopus-0.dll"; fi
...
$(RETRO_TX_BIN): $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_RETRO_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ)
	$(CC) $(CFLAGS) $(EXTRA_LDFLAGS) -o $@ $(OBJ_DIR)/app_desktop_tx.o $(DESKTOP_TX_RETRO_OBJECT) $(DESKTOP_OPUS_OBJECT) $(DESKTOP_LIB) $(CORE_LIB) $(PROTO_LIB) $(DESKTOP_COMMON_OBJECTS) $(TX_UI_NCURSES_OBJ) $(LDLIBS_DESKTOP_RETRO) $(TX_NCURSES_LDLIBS)
ifneq ($(WINDOWS_RUNTIME_DLLS),)
	cp -f $(WINDOWS_RUNTIME_DLLS) $(BIN_DIR)/
endif
	@if [ "$(MINGW_ARCH)" = "mingw32" ] && [ -f "$(BIN_DIR)/libopus.dll" ] && [ ! -f "$(BIN_DIR)/libopus-0.dll" ]; then cp -f "$(BIN_DIR)/libopus.dll" "$(BIN_DIR)/libopus-0.dll"; fi
...
test: dirs $(CORE_LIB) $(PROTO_LIB) $(TEST_BIN) $(TEST_TRANSPORT_UDP_BIN) $(TEST_PCM_RATE_CONVERT_BIN) $(TEST_OPUS_ROUNDTRIP_BIN) $(TEST_NB_CODEC_ADAPTERS_BIN)
	$(TEST_BIN)
