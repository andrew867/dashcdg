...
3. **`handle_*`** routines update `receiver_state`, including **`dashcdg_audio_jitter_insert`** for live audio frames and FEC recoveries.
4. **`dashcdg_rx_drain_media_locked`** calls **`dashcdg_audio_jitter_drain_step`** in a bounded loop; on **APPLY**, decodes and calls **`dashcdg_desktop_audio_queue_frames`** (PortAudio boundary).
5. **Render** path copies `dashcdg_cdg_state` snapshot; **`dashcdg_gl_renderer_render`** builds RGBA via **`dashcdg_cdg_state_to_rgba8`** then uploads **one** `GL_RGBA` texture and draws a textured quad (shader simplified to passthrough).
...
- Main thread: GLUT display (OpenGL).
...
- `libdashcdg_core.a` gains `audio_jitter.o` and `cdg_raster.o`.
- `libdashcdg_desktop.a` gains `desktop_transport_udp.o` via `DESKTOP_APP_OBJECTS` in the Makefile.
