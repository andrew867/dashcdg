...
|-----------|----------------------|-----------------|-------------|-------|
| `bitrate_bps < 96000` | `OPUS_APPLICATION_AUDIO` | **`OPUS_SIGNAL_VOICE`** | `OPUS_BANDWIDTH_FULLBAND` via **AUTO** (`opus_encoder_ctl(..., OPUS_SET_BANDWIDTH(OPUS_AUTO))`) | Speech/karaoke at lower bitrates; constrained VBR stays enabled where already set |
