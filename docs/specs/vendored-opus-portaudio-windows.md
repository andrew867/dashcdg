...
6. **Incremental vendor CMake (default):** `scripts/build_mingw32_p3_opus_portaudio_shared.sh` skips rebuilding **libopus** and/or **libportaudio** when the corresponding DLL is present, the stamp under the prefix matches a hash of this script’s CMake flags (`.dashcdg_p3_opus_build_id` / `.dashcdg_p3_portaudio_build_id`), and the repo’s vendor `CMakeLists.txt` is not newer than the DLL (sources are compared under `audio_modules/.../vendor/`, not a per-run staging copy). Set **`DASHCDG_P3_VENDOR_REBUILD=1`** to force a full rebuild of both codecs. To skip the vendor script entirely (prebuilt DLLs), use **`SKIP_MINGW32_P3_VENDOR=1`** as before.
