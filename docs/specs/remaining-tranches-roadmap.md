...
## Snapshot update: 2026-04-29

- **Enterprise group sync (residual phase)** is specified and test-planned in [`enterprise-group-sync-spec.md`](enterprise-group-sync-spec.md), [`../test/enterprise-group-sync-test-plan.md`](../test/enterprise-group-sync-test-plan.md), and [`../plans/enterprise-group-sync-tranches.md`](../plans/enterprise-group-sync-tranches.md). Prior ad hoc enterprise closeout docs are archived under [`../archive/enterprise-sync-masterplan-2026-04/`](../archive/enterprise-sync-masterplan-2026-04/README.md).
- **Network matrix:** desktop soak hosts are **Ethernet**; **ESP32** is the only **Wi‑Fi** reference device in the enterprise matrix (see spec §5.2).
...
| **Tranche A: baseline convergence + observability** | [../ops/v4-group-playout-sync-rollout.md](../ops/v4-group-playout-sync-rollout.md), [v4-network-stats-and-adaptation.md](v4-network-stats-and-adaptation.md), [`enterprise-group-sync-spec.md`](enterprise-group-sync-spec.md) (residual program) | [v4-group-playout-sync-validation.md](../test/v4-group-playout-sync-validation.md), [v4-network-observability-validation.md](../test/v4-network-observability-validation.md), [`enterprise-group-sync-test-plan.md`](../test/enterprise-group-sync-test-plan.md) | Capture current spread first; complete RX measurement fields before controller logic. **Enterprise:** detrended spread + smoothed controller + gates (2026 spec). |
| **Tranche B: group playout sync / IDMS** | [../ops/v4-group-playout-sync-rollout.md](../ops/v4-group-playout-sync-rollout.md), [v4-group-playout-sync-idms.md](v4-group-playout-sync-idms.md), [`enterprise-group-sync-spec.md`](enterprise-group-sync-spec.md) | [v4-group-playout-sync-validation.md](../test/v4-group-playout-sync-validation.md), [`enterprise-group-sync-test-plan.md`](../test/enterprise-group-sync-test-plan.md) | TX-as-controller first; measurement mode before real target following. **Enterprise tranche order:** [`../plans/enterprise-group-sync-tranches.md`](../plans/enterprise-group-sync-tranches.md). |
...
| **Tranche D.3: PCM SRC and equal-rate sync hardening** | [pcm-libsoxr-desktop-src.md](pcm-libsoxr-desktop-src.md) | [pcm-libsoxr-regression.md](../test/pcm-libsoxr-regression.md), `make test` (`test-pcm-rate-convert`) | Track current desktop contract for in-tree SRC paths and equal-rate sync correction safeguards. |
...
| **Tranche E: V5 simulcast / IGMP / ladder** | [v5-multistream-adaptation-architecture.md](v5-multistream-adaptation-architecture.md), **[v5-broadcast-rack-protocol-spec.md](v5-broadcast-rack-protocol-spec.md)** (umbrella), **[../plans/v5-broadcast-rack-implementation-tranches.md](../plans/v5-broadcast-rack-implementation-tranches.md)** | [v5-broadcast-rack-test-plan.md](../test/v5-broadcast-rack-test-plan.md), v4 soak + future v5-specific matrix | After v4 stability; **P1–P7** tranches: discovery → timeline → clock → FEC → telemetry → wire v5 → IGMP ladder. |
...
1. **Soak environments**: tag results for **both** — **Ethernet + impaired relay (lab)** and, where applicable, **ESP32 on Wi-Fi** — separate threshold rows where they differ. **Desktop enterprise acceptance** uses **wired** hosts; Wi-Fi jitter on desktops is out of matrix unless explicitly testing that path (see [`enterprise-group-sync-spec.md`](enterprise-group-sync-spec.md) §5.2).
