...
| **Slave (ESP32 badge)** | **Preferred:** vendor **[epagris/flexPTP](https://github.com/epagris/flexPTP)** (MIT) — IEEE‑1588 / 802.1AS-oriented library for **lwIP** (and other stacks) on MCUs; port **timestamp hooks** in the Ethernet driver per upstream docs. Map servo output into the same **session timeline** contract as desktop. **Fallback:** thin in-house subset if port cost blocks a release. | **MIT** (flexPTP) + project glue. | Do **not** ship GPL **[ESP1588](https://github.com/leifclaesson/ESP1588)**. flexPTP ships **CMake** and **FreeRTOS**-friendly patterns; ESP‑IDF integration = submodule + `flexptp_options.h` + driver glue. **SBOM:** **V5-BR-CLK-06**. |

**Rationale:** GPL **ESP1588** remains **off-line** behavioral reference only. **[flexPTP](https://github.com/epagris/flexPTP)** gives a **permissive**, MCU-proven PTP stack (lwIP, optional gPTP profile) so the badge path does not require a clean-room reimplementation on day one. The **MIT** TX grandmaster stack ([IEEE1588-PTP](https://github.com/bestvibes/IEEE1588-PTP)) stays the **host GM** integration candidate with attribution.
...
- [epagris/flexPTP](https://github.com/epagris/flexPTP) (MIT) — **ESP32 / MCU PTP slave** integration candidate (lwIP + driver timestamps); see §6.1.
- [leifclaesson/ESP1588](https://github.com/leifclaesson/ESP1588) (GPL‑3.0) — **not** shipped; **behavioral** reference only; superseded for vendoring by flexPTP; see §6.1.
